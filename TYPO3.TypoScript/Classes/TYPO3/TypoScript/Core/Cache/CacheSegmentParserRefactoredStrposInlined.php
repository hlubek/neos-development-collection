 ...
 */
class CacheSegmentParserRefactoredStrposInlined
{
    /**
     * @var string
     */
    ...
    public function __construct($content, $randomCacheMarker = '')
    {
        ...
        while ($nextStartPosition !== false) {
            $part = substr($this->content, $currentPosition, $nextStartPosition - $currentPosition);
            $this->output .= $part;
        ...
        $part = substr($this->content, $currentPosition);
        $this->output .= $part;
        $this->outerSegmentContent .= $part;
...
,        ];
        ...
        $identifier = substr($this->content, $currentPosition, $nextIdentifierSeparatorPosition - $currentPosition);
        $fromPosition = $this->calculateCurrentPosition($nextIdentifierSeparatorPosition);
        $toPosition = $nextSecondIdentifierSeparatorPosition;
        $contextOrMetadata = substr($this->content, $fromPosition, $toPosition - $fromPosition);
    ...
    {
        return substr($this->content, $fromPosition, ($toPosition === null ? $toPosition : $toPosition - $fromPosition));
    }
    ...
    {
        return $position + 1 + strlen($this->randomCacheMarker);
    }
